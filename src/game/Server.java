...
public class Server {
	...
	public class ClientHandler extends Thread{
		...
		ClientHandler(Socket socket) throws ClassNotFoundException, IOException, InterruptedException{
			...
			HumanSnake hs = new HumanSnake(id,board); 	/// while (true) englobava as a criacao de um HumanPlayer -errado pq por cada cliente que se liga ao servidor apenas � criado um humanplayer
			id++;
			board.addSnake(hs);
			this.hs=hs;
		...
		public void handleIn() throws ClassNotFoundException, IOException, InterruptedException{
			while(true) {
				String c = (String) in.readObject(); // bufferedReader como input, transformar o texto em direcao depois de ler, usar ENUM
	...
	public class GameUpdater extends Thread {
		...
		GameUpdater(Socket socket) throws IOException, InterruptedException, ClassNotFoundException{
			...
			this.initializeOut(this.clientSocket);   //inicializacao dos canais de input e output do cliente com a socket do cliente como parametro
